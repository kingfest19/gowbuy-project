...
class DownloadRecoveryCodesView(TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not self.authenticator:
            messages.info(
                request,
                _("You do not have any recovery codes to download."),
            )
            return HttpResponseRedirect(reverse("mfa_generate_recovery_codes"))
        self.unused_codes = self.authenticator.wrap().get_unused_codes()
        if not self.unused_codes:
            messages.warning(
                request,
                _(
                    "You have used all of your recovery codes. Please generate a new set."
                ),
            )
            return HttpResponseRedirect(reverse("mfa_generate_recovery_codes"))
...
class ViewRecoveryCodesView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ret.update(
            {
                "unused_codes": self.authenticator.wrap().get_unused_codes(),
